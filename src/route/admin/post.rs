...
pub async fn new_form(
    ...
    //let _model = controller::post::create(&db, &form.title, &form.description, form.category_id).await.unwrap();
    //Ok(utils::see_other("/admin"))
    todo!()
}
...
pub async fn new(
    ...
    let _model = controller::post::create(&db, &form.title, &form.description, form.category_id)
        .await
        .unwrap();
    Ok(utils::see_other("/admin"))
}
